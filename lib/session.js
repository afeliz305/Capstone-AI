...
function createSessionService({ resolveAccount = null, enableDemo = false } = {}) {
  ...
  function demoAllowed(request) {
    ...
    if (!enableDemo || resolveAccount || process.env.NODE_ENV === "production") return false;
  ...
  function changeDemo(request, response, action) {
    ...
    if (action === "end") {
      ...
      response.setHeader("Set-Cookie", `${COOKIE_NAME}=; HttpOnly; SameSite=Strict; Path=/api; Max-Age=0`);
      return;
    }
    ...
    response.setHeader("Set-Cookie", `${COOKIE_NAME}=${token}; HttpOnly; SameSite=Strict; Path=/api; Max-Age=3600`);
  }
