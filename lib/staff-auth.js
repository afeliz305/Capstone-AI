...
async function setStaffPassword(email, password, { file = credentialsFile } = {}) {
  const member = memberFor(email);
  if (!member) throw new Error("That email is not on the staff list.");
  if (typeof password !== "string" || password.length < 12 || password.length > 128) {
    throw new Error("Use a separate prototype password of 12 to 128 characters.");
  }
  const accounts = await readCredentials(file);
  const salt = randomBytes(16).toString("hex");
  const hash = (await scrypt(password, salt, 64)).toString("hex");
  accounts[member.email] = { salt, hash, changedAt: new Date().toISOString() };
...
function createStaffAuth({ file = credentialsFile, now = Date.now, sessionMs = SESSION_MS } = {}) {
  ...
  const versionFor = (record) => createHash("sha256").update(`${record.salt}:${record.hash}`).digest("hex");
  ...
  function setCookie(request, response, token, maxAge) {
    response.setHeader("Set-Cookie", `${COOKIE}=${token}; HttpOnly; SameSite=Strict; Path=/api; Max-Age=${maxAge}${request.socket.encrypted ? "; Secure" : ""}`);
  }
  ...
  async function login(request, response, input) {
    ...
    const usable = validRecord(record);
    ...
    if (!timingSafeEqual(actual, expected) || !usable || password.length < 12) {
    ...
    sessions.set(token, { email: member.email, expiresAt, version: versionFor(record) });
    setCookie(request, response, token, Math.ceil(sessionMs / 1000));
    return { staff: member, expiresAt, members: STAFF };
  }
  async function current(request) {
    ...
    if (!validRecord(record) || versionFor(record) !== session.version) {
      sessions.delete(token);
      throw fail("Unauthorized access. Your staff session has ended. Sign in again.", 401);
    }
    return { staff: memberFor(session.email), expiresAt: session.expiresAt, members: STAFF };
  }
