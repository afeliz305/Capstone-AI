...
The September 26 hosted connector release adds opt-in live dashboard access to the Ocelot prototype through the unpacked Chrome extension. Testers grant the FIU portal and the owner's hosted MIRA page separately; private questions and extracted records remain in browser memory and never enter Supabase. See [private portal connector setup](docs/LOCAL_PORTAL_CONNECTOR.md#hosted-ocelot-mode).

**September 26 local connector upgrade:** `npm.cmd run demo:portal` now uses a narrowly scoped Manifest V3 Chrome extension and loopback bridge; it does not require Chrome remote debugging or silently use MCP. The MCP adapter remains available only through `npm.cmd run demo:portal:mcp`. MIRA maps all 18 internal dashboard sections, supports on-demand read-only extraction for 16 authorized sections, intentionally excludes the Classmates and Alumni directories, and can read only a conversation the student deliberately opened after the student enables the message-content scope. Private snapshots remain memory-only for five minutes and refresh without revoking extension permission/trust. Review, Retrospective and Daily Scrum templates are now searchable; a real cadence/submission conflict is disclosed, while global Verify, Definition of Done and acceptance-criteria rules are reported as not found after inspection. This is local source work only: no Ocelot package, deployment, Git push, portal change, or published extension is included. See [normal extension setup and complete coverage](docs/LOCAL_PORTAL_CONNECTOR.md) and [source acceptance](docs/MIRA_SOURCE_COVERAGE.md).
...
**Local portal personalization (September 25):** `npm.cmd install --include=dev`, then `npm.cmd run demo:portal` (or `scripts\Start Capstone Portal Demo.cmd`) starts the read-only loopback demo on port 3005. After one fresh Overview verification, MIRA lazily reads only the approved section a student deliberately asks about: Overview, their own Team, Standing, Grade, or Messages channel metadata. Messages never opens a conversation. Each source keeps its section/subview and retrieval metadata; private snapshots expire on the original five-minute deadline and stay out of Supabase, tickets, files, analytics and model providers. Focused connector/MIRA/syllabus/chat tests passed 54/54; the full project passed 263 with one intentionally skipped hosted-PHP test. The real application-owned Chrome connection still stops at **Browser connection unavailable**, so live personal retrieval/navigation acceptance remains pending the Chrome approval/handshake. [Setup, section coverage, privacy boundaries, tests and live status](docs/LOCAL_PORTAL_CONNECTOR.md). This connector is never included in public Ocelot.
