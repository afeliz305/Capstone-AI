...
const {EXTENSION_ORIGIN}=require('./extension-bridge');
...
function createPortalServer({root,sourceRoot,service,extensionBridge=null,pairingCode=opaque(),port=3005,now=Date.now}) {
  ...
  async function body(req,max=4096) {let size=0,chunks=[];for await(const c of req){size+=c.length;if(size>max)throw new Error('size');chunks.push(c);}const value=JSON.parse(Buffer.concat(chunks).toString('utf8'));if(!value||typeof value!=='object'||Array.isArray(value))throw new Error();return value;}
  const server=http.createServer(async(req,res)=>{
    ...
    if(url.pathname.startsWith('/__mira_extension/')){
      const extensionOrigin=req.headers.origin||'';
      if(!extensionBridge||req.method!=='POST'||!EXTENSION_ORIGIN.test(extensionOrigin)||!/^application\/json(?:;|$)/i.test(req.headers['content-type']||'')||url.search)return send(res,403,{error:'A paired Chrome extension request is required.'});
      if(!budget('extension',240))return send(res,429,{error:'Please wait before retrying.'});
      let data;try{data=await body(req,2*1024*1024);}catch{return send(res,400,{error:'Invalid or oversized extension request.'});}
      const operation=url.pathname.slice('/__mira_extension/'.length);
      try{
        let result;
        if(operation==='pair')result=extensionBridge.pair(extensionOrigin,data);
        else if(operation==='next')result=extensionBridge.next(extensionOrigin,data);
        else if(operation==='result')result=extensionBridge.complete(extensionOrigin,data);
        else if(operation==='revoke'){extensionBridge.authenticate(extensionOrigin,data);extensionBridge.revoke();result={disconnected:true};}
        else return send(res,404,{error:'Unsupported extension operation.'});
        return send(res,200,result);
      }catch(error){return send(res,error instanceof PortalError?401:400,{error:error instanceof PortalError?error.message:'Invalid extension request.'});}
    }
    ...
    if(url.pathname.startsWith('/__portal/')) {
      ...
      try {
        ...
        if(operation==='configure') result=service.configure(session.id,data);
  ...
  server.on('close',()=>{clearInterval(sweep);sessions.clear();bootstrapTokens.clear();extensionBridge?.revoke('helper-stopped');if(service.owner)void service.disconnect(service.owner);});
  return {server,pairingCode,pairingExpiresAt,origin,extensionPairingCode:extensionBridge?.pairingCode||null,extensionPairingExpiresAt:extensionBridge?.pairingExpiresAt||null};
}
