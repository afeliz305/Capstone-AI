(() => {
  ...
  var require_dom_reader = __commonJS({
    "server/portal-local/dom-reader.js"(exports, module) {
      ...
      function readPortalGuard2() {
        ...
        const selected = document.querySelector("main .sidebar .nav-item.on");
        return { state: "present", documentId: performance.timeOrigin, active: selected?.textContent?.trim() || null, route: selected?.getAttribute("data-v") || null, editable: !!document.activeElement?.matches('input,textarea,select,[contenteditable="true"]') };
      }
      ...
      async function navigatePortalSection2(section) {
        if (location.origin !== "https://capstone.cs.fiu.edu" || location.pathname !== "/portal" || location.search) return { state: "connection-lost" };
        const routes = { Overview: "home", Messages: "messages", "Start here": "orientation", Team: "team", Standing: "compare", Grade: "mygrade", Classmates: "netclass", "Alumni directory": "netdir", Connections: "netconn", Opportunities: "netopp", "Team contacts": "netteam", "AI Anchors": "aidir", Record: "caprecord", Showcase: "myshowcase", Letters: "myletters", "Request a letter": "reqletter", Resources: "resources", "Brand & templates": "brandhub" };
      ...
      function readPortalCapabilities2() {
        if (location.origin !== "https://capstone.cs.fiu.edu" || location.pathname !== "/portal" || location.search) return { state: "connection-lost", sources: [] };
        const routes = { Overview: "home", Messages: "messages", "Start here": "orientation", Team: "team", Standing: "compare", Grade: "mygrade", Classmates: "netclass", "Alumni directory": "netdir", Connections: "netconn", Opportunities: "netopp", "Team contacts": "netteam", "AI Anchors": "aidir", Record: "caprecord", Showcase: "myshowcase", Letters: "myletters", "Request a letter": "reqletter", Resources: "resources", "Brand & templates": "brandhub" };
        const excluded = /* @__PURE__ */ new Set(["Classmates", "Alumni directory"]), nodes = [...document.querySelectorAll("main .sidebar .nav-item[data-v]")], sources = [];
        for (const [section, route] of Object.entries(routes)) {
          const node = nodes.find((el) => el.getAttribute("data-v") === route);
          sources.push({ section, route, discovered: !!node, accessible: !!node && !node.disabled, inspected: false, extractionSupported: !excluded.has(section), currentlyLoaded: node?.classList.contains("on") || false, searchable: !excluded.has(section), navigation: "parent", blocked: excluded.has(section) ? "Unrelated student/alumni directory records are intentionally excluded." : null });
        }
        for (const link of [...document.querySelectorAll("main .sidebar a[href]")]) {
          let url;
          try {
          ...
          if (!["https://fiu.instructure.com", "https://capstone.cs.fiu.edu"].includes(url.origin)) continue;
          sources.push({ section: (link.textContent || "External resource").trim().slice(0, 80), route: null, discovered: true, accessible: true, inspected: false, extractionSupported: false, currentlyLoaded: false, searchable: false, navigation: "exact", url: url.origin + url.pathname, blocked: url.origin === location.origin ? null : "Separate site permission and authentication are required." });
        }
        return { state: "present", sources };
      }
      function readPortalSection2(input) {
        const section = typeof input === "string" ? input : input?.section, messageContent = typeof input === "object" && input?.messageContent === "selected-conversation";
        if (location.origin !== "https://capstone.cs.fiu.edu" || location.pathname !== "/portal" || location.search) return { state: "connection-lost" };
        const routes = { Overview: "home", Messages: "messages", "Start here": "orientation", Team: "team", Standing: "compare", Grade: "mygrade", Classmates: "netclass", "Alumni directory": "netdir", Connections: "netconn", Opportunities: "netopp", "Team contacts": "netteam", "AI Anchors": "aidir", Record: "caprecord", Showcase: "myshowcase", Letters: "myletters", "Request a letter": "reqletter", Resources: "resources", "Brand & templates": "brandhub" };
        ...
        const safeUrl = (value) => {
          try {
            ...
            if (url.origin !== location.origin || url.username || url.password || url.search) return location.origin + "/portal";
        ...
        const add = (kind, heading, text, subview, sourceTimestamp = null, coverage = "", sourceUrl = location.origin + "/portal") => {
        ...
        const extractBoard = () => {
          for (const column of [...content.querySelectorAll(".sb-col")].slice(0, 12)) {
            const heading = clean(column.querySelector("h3,h4,.sb-col-title")?.textContent) || "Sprint board column", cards = [...column.querySelectorAll(".sb-card-view")].slice(0, 40);
            if (!cards.length) add("sprint-board", heading, safeText(column), "Sprint board");
            for (const card of cards) {
              const title = clean(card.querySelector(".sb-card-title")?.textContent) || "Sprint task", body = [card.querySelector(".sb-card-desc"), card.querySelector(".sb-card-meta"), card.querySelector(".sb-crit"), card.querySelector(".sb-card-history")].map((el) => clean(el?.textContent)).filter(Boolean).join("\n");
              add("task", title, body || safeText(card), "Sprint board \xB7 " + heading, null, "Visible read-only card description, metadata, criteria, evidence/history, and status only. Edit, move, archive, approval, and Product Owner controls are excluded.");
            }
          }
        };
        const extractCeremonies = () => {
          for (const panel of [...content.querySelectorAll('[id^="sb-ceremony-"]')].slice(0, 20)) add("ceremony", clean(panel.querySelector("h3,h4")?.textContent) || "Sprint ceremony", safeText(panel), "Sprint Review / Retrospective");
          for (const panel of [...content.querySelectorAll('[class*="standup"]')].filter((el) => el.tagName !== "FORM" && !el.closest("form")).slice(0, 20)) add("standup", clean(panel.querySelector("h3,h4")?.textContent) || "Stand-up information", safeText(panel), "Stand-up");
          for (const table of [...content.querySelectorAll("table")].slice(0, 12)) add("schedule", clean(table.querySelector("caption")?.textContent) || "Schedule or deadline table", safeText(table), "Schedule");
        };
        if (section === "Overview") {
        ...
        if (section === "Team") {
          ...
          for (const card of [...root.querySelectorAll(".card")].slice(0, 20)) {
            if (!visible(card) || card.classList.contains("collapsed")) continue;
            const heading = clean(card.querySelector("h2,h3,h4")?.textContent) || "Team details", kind = /product owner/i.test(heading) ? "product-owner" : /lead/i.test(heading) || card.querySelector(".pc-leader,.pc-leader-confirmed") ? "leadership" : "team";
        ...
        if (section === "Standing") {
          for (const card of [...content.querySelectorAll(":scope > .card,.card")].slice(0, 10)) {
            ...
            let text = safeText(card);
            const why = card.querySelector('.whyi[type="button"]:not([form])');
        ...
        if (section === "Grade") {
          const root = content.querySelector("#mygradebox") || content, details = [...root.querySelectorAll("details.gpast")].slice(0, 5), states = details.map((el) => el.open);
          details.forEach((el) => {
            el.open = true;
          });
          try {
            for (const card of [...root.querySelectorAll(":scope > .card,.card")].slice(0, 20)) {
              ...
              add("grade", clean(card.querySelector("h2,h3,h4")?.textContent) || "My grade", safeText(card), "Current grade");
            }
            for (const detail of details) add("past-grade", clean(detail.querySelector("summary")?.textContent) || "Past term grade details", safeText(detail), "Past term");
          } finally {
            details.forEach((el, index) => {
              el.open = states[index];
            });
          }
        }
        if (section === "Messages") {
          const side = content.querySelector("#csSide");
          if (!side) return { state: "verified", section, records: [], coverage: "Messages was present, but its channel sidebar was unavailable. No conversation was selected or read." };
          for (const channel of [...side.querySelectorAll(".cs-chan")].slice(0, 60)) {
            if (!visible(channel)) continue;
            const directName = [...channel.childNodes].filter((node) => node.nodeType === 3).map((node) => node.textContent).join(" "), name = clean(channel.getAttribute("aria-label") || channel.getAttribute("data-channel-name") || channel.querySelector(".cs-name,.name,.label,.title,[data-channel-name]")?.textContent || directName).slice(0, 180);
            if (!name) continue;
            const unread = channel.classList.contains("unread"), count = clean(channel.querySelector(".cs-count")?.textContent);
            add("messages", name, "Channel: " + name + "\nUnread: " + (count || (unread ? "yes" : "no")), "Channel list", null, "Channel metadata only; no channel was selected automatically.");
          }
          if (messageContent) {
            const conversation = clean(content.querySelector(".cs-head .cs-title,#csHead .cs-title")?.textContent) || "Selected conversation";
            for (const message of [...content.querySelectorAll("#csMsgs .cs-msg")].filter(visible).slice(-100)) {
              const author = clean(message.querySelector(".cs-who")?.textContent), time = clean(message.querySelector(".cs-t")?.textContent), body = clean(message.querySelector(".cs-mtext")?.textContent);
              if (body) add("message-content", conversation, ["Author: " + (author || "Unknown"), "Time: " + (time || "Not displayed"), "Message: " + body].join("\n"), "User-selected visible conversation", null, "Only messages already visible in the conversation deliberately opened by the student. Threads, hidden history, composer, typing, reactions and actions are excluded.");
            }
          }
        }
        ...
        if (["Connections", "Opportunities", "Team contacts", "AI Anchors", "Record", "Showcase", "Letters", "Request a letter", "Resources", "Brand & templates"].includes(section)) {
          ...
          for (const card of [...content.querySelectorAll(".card")].filter(visible).slice(0, 40)) add(kinds[section], clean(card.querySelector("h1,h2,h3,h4")?.textContent) || section, safeText(card), section);
          if (["Resources", "Brand & templates", "Showcase"].includes(section)) for (const link of [...content.querySelectorAll("a[href]")].filter(visible).slice(0, 100)) {
            const heading = clean(link.textContent) || clean(link.closest(".card")?.querySelector("h2,h3,h4")?.textContent) || "Linked resource";
            if (heading) add("resource-link", heading, "Verified dashboard resource link: " + heading, section + " links", null, "The destination was observed in the signed-in dashboard. Document contents are separate and may require extraction.", link.href);
          }
        }
        const limits = { Overview: "Visible approved Overview cards, sprint board read-only views, ceremonies, stand-up information, and schedules. Editing controls, hidden fields, and unrelated teams are excluded.", Team: "The signed-in account\u2019s own Team view, visible sprint board cards, criteria, evidence/history, ceremonies, members, leadership, and Product Owner information. Unrelated teams and all editing/workflow controls are excluded.", Standing: "Visible standing and trend information; no control is activated.", Grade: "The signed-in account\u2019s visible current grade cards and temporarily disclosed past-term details. No prediction or other student record is read.", Messages: messageContent ? "Channel metadata plus only the currently visible conversation deliberately opened by the student. No channel is selected automatically." : "Channel metadata and unread indicators only. Conversation bodies are not read.", "Start here": "Visible onboarding status and instructions. Review/edit forms are not opened.", Connections: "The signed-in student\u2019s visible connection summary only.", Opportunities: "Visible opportunities and instructions only; no application is submitted.", "Team contacts": "Contacts intentionally shared with the student\u2019s own team. Unrelated directory profiles are excluded.", "AI Anchors": "Visible AI Anchor guidance and results; search controls are not used.", Record: "The signed-in student\u2019s visible Capstone record.", Showcase: "The signed-in student\u2019s visible showcase status, readiness guidance, and same-origin resource destinations. No submission is made.", Letters: "The signed-in student\u2019s visible recommendation-letter status.", "Request a letter": "Static request instructions only. Draft/form values are excluded and no request is submitted.", Resources: "Visible resource descriptions and verified same-origin links. Linked contents require separate extraction.", "Brand & templates": "Visible brand guidance and verified same-origin links. Linked contents require separate extraction.", Classmates: "Discovered and navigable, but unrelated student directory records are intentionally excluded.", "Alumni directory": "Discovered and navigable, but unrelated alumni directory records are intentionally excluded." };
        return { state: "verified", section, records: records.slice(0, 240), coverage: limits[section] || "Visible read-only content from the selected dashboard section." };
      }
      var readPortalDom = readPortalSection2;
      module.exports = { SECTION_ROUTES: SECTION_ROUTES2, INTENTIONALLY_EXCLUDED, readPortalDom, readPortalGuard: readPortalGuard2, readPortalIdentity, readPortalCapabilities: readPortalCapabilities2, navigatePortalSection: navigatePortalSection2, readPortalSection: readPortalSection2 };
    }
  });

  // portal-extension/content-entry.js
  var { SECTION_ROUTES, readPortalGuard, readPortalCapabilities, navigatePortalSection, readPortalSection } = require_dom_reader();
  var PORTAL = "https://capstone.cs.fiu.edu/portal";
  var documentId = crypto.randomUUID();
  var exactPortal = () => location.origin === "https://capstone.cs.fiu.edu" && location.pathname === "/portal" && !location.search;
  ...
  async function freshIdentity() {
    ...
    let response, text;
    try {
      response = await fetch(PORTAL, { method: "GET", credentials: "same-origin", cache: "no-store", redirect: "follow", headers: { Accept: "text/html" } });
      ...
      try {
        ...
        if (message.operation === "guard") result = { ...readPortalGuard(), documentId };
          ...
          if (!Object.hasOwn(SECTION_ROUTES, section)) result = { state: "section-denied" };
            ...
            const moved = await navigatePortalSection(section);
            result = { ...moved, guidance: "The verified parent section is open. Open a nested detail manually when no stable deep link exists." };
          }
