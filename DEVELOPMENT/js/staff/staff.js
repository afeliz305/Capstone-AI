(() => {
  ...
  ticketForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const response = await api.fetch("/api/staff/tickets", {
        method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
      const created = await api.readJson(response);
      if (epoch !== authEpoch || requestFailed(response, created)) return;
      if (!/^CAP-\d+$/.test(created.id)) throw new Error("No valid ticket receipt was returned. Check the queue before retrying.");
  ...
  async function updateTicket(ticket, changes, actions) {
    ...
    try {
      const response = await api.fetch("/api/tickets/" + encodeURIComponent(ticket.id), {
        method: "PATCH", credentials: "same-origin",
        headers: { "Content-Type": "application/json" }, body: JSON.stringify(api.storageMode === "supabase"
          ? { ...changes, expectedRevision: ticket.revision || 0, requestId: crypto.randomUUID() } : changes)
      });
      const updated = await api.readJson(response);
  ...
  function renderTicket(ticket, grouped = false) {
    ...
    const identityLabels = { "portal-session": "From signed-in account", "demo-session": "Sample account · demo only", "sample-seed": "Generated test data", "staff-session": "Created by signed-in staff", "staff-email-demo": "Staff email-only demo · identity unverified", manual: "Manually entered contact" };
    ...
    if (ticket.attachments?.length) {
      ...
      for (const file of ticket.attachments) {
        ...
        link.href = api.url("/api/tickets/" + encodeURIComponent(ticket.id) + "/attachments/" + encodeURIComponent(file.id));
        link.download = file.name;
        if (api.download) link.addEventListener("click", async event => {
          event.preventDefault();
          try { await api.download("/api/tickets/" + ticket.id + "/attachments/" + file.id, file.name); }
  ...
  async function loadTickets() {
    ...
    try {
      const response = await api.fetch("/api/tickets", { credentials: "same-origin", cache: "no-store" });
  ...
  async function checkSession() {
    const epoch = authEpoch;
    try {
      const response = await api.fetch("/api/staff/session", { credentials: "same-origin", cache: "no-store" });
  ...
  if (backupButton) backupButton.addEventListener("click", async () => {
    ...
    try { await api.download("/api/browser-backup", "capstone-browser-tickets-" + new Date().toISOString().slice(0, 10) + ".json"); }
    catch (error) { (staff ? status : loginStatus).textContent = error.message; }
