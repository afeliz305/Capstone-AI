...
September 25 local private portal demo: install dependencies with `npm.cmd install --include=dev`, then run `npm.cmd run demo:portal` or double-click `scripts\Start Capstone Portal Demo.cmd`. Keep the terminal running and open `http://127.0.0.1:3005/Capstone%20-%20AI/` directly. Choose **Connect my portal**; the page normally establishes its local application session automatically, while the single-use terminal code remains under **Advanced setup**. Keep a dedicated signed-in Chrome Overview tab, manually enable/approve incoming debugging, select the intended tab, and verify it. Initial verification and **Verify again** reload that dedicated tab once; **Refresh information** and ordinary questions do not reload it, and there is no 20-second loop. `ERR_CONNECTION_REFUSED` means the local process stopped. This adapter is separate from public Ocelot/Supabase staff login and never shares private excerpts with tickets. Connector/chat tests passed 34/34 and the full project passed 252/252; real Overview acceptance is still pending. [Complete setup, states, privacy, limits and evidence](LOCAL_PORTAL_CONNECTOR.md).
