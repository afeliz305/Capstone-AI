...
September 26 hosted connector correction: the Ocelot package now includes a memory-only extension bridge for live personal dashboard questions. Reload the unpacked `DEVELOPMENT\portal-extension` extension, grant both **Portal access** and **Hosted MIRA access**, reload the signed-in portal and hosted Ocelot tabs once, and choose **Connect my portal** on Ocelot. Hosted use does not require `npm.cmd run demo:portal`; the local command remains available for development. Personal source text/questions/answers are not sent to Supabase and expire from page memory after five minutes. See [the exact permissions and setup](LOCAL_PORTAL_CONNECTOR.md#hosted-ocelot-mode).

September 26 local portal connector: the normal private demo now uses the unpacked Manifest V3 extension in `DEVELOPMENT\portal-extension`; it no longer asks for Chrome remote-debugging approval. Run `npm.cmd install --include=dev`, then `npm.cmd run demo:portal`, load the extension through `chrome://extensions`, grant its optional Capstone portal access, and pair with the separate single-use extension code printed in the terminal. The app verifies the current FIU session independently of Overview and loads only the relevant authorized section on demand. Message text is off by default and is limited to a conversation the student deliberately opened. Private content remains in helper memory for five minutes. Use `npm.cmd run demo:portal:mcp` only for the explicit older development fallback. Full steps, permissions, coverage and limits are in [LOCAL_PORTAL_CONNECTOR.md](LOCAL_PORTAL_CONNECTOR.md). The Node helper remains local-only; the public package now contains a separate browser-memory hosted bridge that uses the same extension after the Hosted MIRA permission is granted.
