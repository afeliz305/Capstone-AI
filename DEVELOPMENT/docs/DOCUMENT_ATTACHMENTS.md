...
`GET /api/tickets/:ticketId/attachments/:attachmentId` requires a valid server-issued staff session and checks that the file belongs to that ticket, then serves it as a forced download with no-store, nosniff, sandbox, and same-origin resource headers. In the current email-only test mode, anyone knowing an approved email can obtain that session; it does not verify identity. It never uses the original filename as a storage path. A missing file returns 404 without rewriting the ticket. Existing tickets without an attachments field continue to work.

## Before production

This remains a prototype, with Ocelot shared testing pending actual backend verification. The staff queue and download route require a staff session; the temporary email-only mode is intentionally not identity verification. See [staff access](STAFF_ACCESS.md). All five demo identities can access all team tickets and downloads. Assignment and the instructor-privacy preference do not restrict access between staff. Current testing is explicitly fictional-data-only. Do not upload real student records, medical absence notes, credentials, or other sensitive documents. Track official integration and release requirements in [the deployment plan](DEPLOYMENT_AND_INTEGRATION_PLAN.md).
