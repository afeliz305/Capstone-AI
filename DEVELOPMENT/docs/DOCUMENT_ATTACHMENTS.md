...
This remains a prototype, with Ocelot shared testing pending actual backend verification. The staff queue and download route require a staff session; the temporary email-only mode is intentionally not identity verification. See [staff access](STAFF_ACCESS.md). All six demo identities can access all team tickets and downloads. Assignment and the instructor-privacy preference do not restrict access between staff. Current testing is explicitly fictional-data-only. Do not upload real student records, medical absence notes, credentials, or other sensitive documents. Track official integration and release requirements in [the deployment plan](DEPLOYMENT_AND_INTEGRATION_PLAN.md).
