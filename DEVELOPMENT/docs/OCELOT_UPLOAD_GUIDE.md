...
September 26 hosted live-data correction: the generated Supabase folder now contains `js/hosted/portal-client.bundle.js`. The Capstone - AI extension version 0.3.0 has separate explicit grants for the FIU portal and the owner's hosted Ocelot app. After uploading, reload the unpacked extension from `DEVELOPMENT\portal-extension`, grant both permissions in its popup, then reload the portal and Ocelot tabs once. The Ocelot page can then answer supported personal questions from the tester's own signed-in portal session without the local Node helper. Private dashboard content and personal chat remain in the Ocelot tab's memory for at most five minutes and are intercepted before Supabase search. The extension is still required on every tester's computer; uploading the website alone cannot read a portal session.

September 25 syllabus PDF update: the only visible sidebar syllabus link now opens `documents/Fall-Term-2026-CIS-4951-RVC-Capstone-II-public.pdf` in a new tab. This is a flattened 26-page public copy of the supplied syllabus; its private Zoom join URL and mobile number are removed. The complete suite passed 260/260 runnable tests with the optional PHP hosted integration skipped. The current generated Supabase upload contains 22 manifest-matched public files. Upload the entire generated **Capstone - AI** folder so the new `documents` folder is transferred. MIRA still uses `pages/syllabus.html` for searchable, anchored source citations. The owner-supplied original in Downloads is unchanged and is not uploaded.
...
September 25 local portal connector history: `demo:portal` remains a separate owner-run loopback demo, and its Node helper, `js/local/portal-client.js`, terminal codes, and debugging endpoints are never uploaded. The September 26 hosted bridge supersedes the former navigation-only Ocelot limitation: the uploaded browser client can communicate with the explicitly installed extension, while personal content remains outside Supabase and tickets. [Current setup, architecture and limits](LOCAL_PORTAL_CONNECTOR.md).
