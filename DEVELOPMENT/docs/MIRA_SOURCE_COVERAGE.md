...
Updated September 25, 2026. This is the evidence and acceptance record for the existing Capstone - AI/MIRA chatbot. It does not authorize production deployment, expand the Overview-only portal connector, or convert prototype notes into course policy.
...
| Local verified Overview | Development-only port-3005 connector, bound to the selected current account and browser context. | Five-minute explicit verification lease; content retrieval timestamp returned per source. | Only approved visible Overview cards and an optional Messages sidebar indicator. No grades, messages, other students, hidden cards or arbitrary portal pages. |

The public index remains the primary public-site retrieval source. Reviewed Fall 2026 syllabus entries take precedence for that exact course/term. Personal Overview evidence stays in local memory and never enters the public index, Supabase, tickets, analytics, or a model provider.
...
| Grading | Fall 2026 syllabus `syllabus-grading` / `syllabus-grade-scale`, pages 7-16; portal Grade navigation | Public criteria + sign-in navigation | Criteria/scale supported. Future grade prediction and another student's records prohibited. Grade is outside the approved Overview-only connector scope. | Syllabus grading anchor; portal → Grade |
...
| 12 | `partial` | `syllabus-grading` + portal Grade navigation | Grading section; portal → Grade | Pass | No future-grade prediction; posted Grade remains outside Overview-only scope. |
...
Automated tests also cover the seven requested natural-language variants, previous-term rejection, current-sprint ambiguity, Review/Retro separation, Verify/Done separation, stand-up-frequency qualification, explicit source follow-ups, prompt-injection resistance, other-student privacy and absence of paid-AI behavior. On September 25, 2026, the full Node/browser/Supabase/PHP suite passed 261 of 261 tests with Node 24.15.0 and the local PHP 8.4.25 test runtime.
