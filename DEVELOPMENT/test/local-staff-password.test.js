...
async function login(auth, req = request(), email = STAFF.find(member => member.email === "afeli016@fiu.edu").email, password = shortPassword) {
  let cookie;
...
test("short staff passwords require explicit local-only setup and preserve other accounts", async (t) => {
  ...
  await assert.rejects(setStaffPassword(STAFF.find(member => member.email === "afeli016@fiu.edu").email, shortPassword, { file }), /12 to 128/);
  await assert.rejects(setStaffPassword(STAFF.find(member => member.email === "afeli016@fiu.edu").email, "short", { file, localTestOnly: true }), /8 to 128/);
  await setStaffPassword(STAFF.find(member => member.email === "afeli016@fiu.edu").email, shortPassword, { file, localTestOnly: true });
  ...
  assert.equal(accounts[STAFF.find(member => member.email === "afeli016@fiu.edu").email].localTestOnly, true);
...
test("local-only passwords and sessions reject remote or non-localhost requests", async (t) => {
  ...
  assert.equal(signedIn.result.staff.email, STAFF.find(member => member.email === "afeli016@fiu.edu").email);
  assert.equal((await auth.current(request({ headers: { cookie: signedIn.cookie } }))).staff.email, STAFF.find(member => member.email === "afeli016@fiu.edu").email);
  await assert.rejects(login(auth, request(), STAFF.find(member => member.email === "afeli016@fiu.edu").email, "incorrect"), { statusCode: 401 });
  ...
  assert.equal((await login(auth, request({ socket: { remoteAddress: "::1" }, headers: { host: "[::1]:3000" } }))).result.staff.email, STAFF.find(member => member.email === "afeli016@fiu.edu").email);
});
...
test("production rejects local test setup, login, and previously issued sessions", async (t) => {
  ...
  try {
    ...
    await assert.rejects(setStaffPassword(STAFF.find(member => member.email === "afeli016@fiu.edu").email, shortPassword, { file, localTestOnly: true }), /disabled in production/);
...
test("normal password reset removes the local-only exception and revokes old sessions", async (t) => {
  ...
  await setStaffPassword(STAFF.find(member => member.email === "afeli016@fiu.edu").email, shortPassword, { file, localTestOnly: true });
  const session = await login(auth);
  await setStaffPassword(STAFF.find(member => member.email === "afeli016@fiu.edu").email, regularPassword, { file });
  assert.equal(JSON.parse(await fs.readFile(file, "utf8")).accounts[STAFF.find(member => member.email === "afeli016@fiu.edu").email].localTestOnly, undefined);
  await assert.rejects(auth.current(request({ headers: { cookie: session.cookie } })), { statusCode: 401 });
  await assert.rejects(login(auth), { statusCode: 401 });
  assert.equal((await login(auth, request(), STAFF.find(member => member.email === "afeli016@fiu.edu").email, regularPassword)).result.staff.email, STAFF.find(member => member.email === "afeli016@fiu.edu").email);
});
