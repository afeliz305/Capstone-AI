...
async function mount({ createResponse, workspaceResponse, authResponse, localStorage, readFile = (reader, file) => { reader.result = "data:text/plain;base64," + file.data; reader.onload(); }, confirm = () => false, redirectFactory = helpers.createIdleRedirect, initialTickets = records } = {}) {
  class Element {
...
test("staff document picker adds and removes files, submits bytes, and exposes saved downloads", async () => {
  ...
  assert.match(links[0].href, /^\/api\/tickets\/CAP-9001\/attachments\/[0-9a-f-]{36}$/);
