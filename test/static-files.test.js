...
test("root homepage and staff page serve with all referenced local assets in their new folders", async () => {
  ...
  const expectedTypes = { ".html": "text/html", ".css": "text/css", ".js": "text/javascript", ".svg": "image/svg+xml", ".woff2": "font/woff2" };
  ...
  for (const asset of ["css/fonts/mulish-var.woff2", "css/images/FIU_mark_white.svg", "css/images/icons.svg", "js/chat/capstone-chat.js", "js/staff/staff.js", "js/shared/contact-policy.js"]) assert.ok(checked.has(`${base}/${asset}`), asset);
...
test("root-level serving never exposes backend code, guides, credentials, data, or arbitrary files", async () => {
  for (const route of [
    "/server/server.js", "/server/lib/staff-auth.js", "/data/capstone-knowledge.json",
    ...
    "/test/static-files.test.js", "/scripts/set-staff-password.js", "/css/", "/js/",
...
test("reorganized documentation links resolve and startup and private data locations remain correct", () => {
  ...
  const documents = ["README.md", "AGENTS.md", ...fs.readdirSync(path.join(root, "docs")).filter(file => file.endsWith(".md")).map(file => "docs/" + file)];
