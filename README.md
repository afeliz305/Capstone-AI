...
The staff sign-in page remembers the last successfully signed-in email on this browser and prefills it after logout or a later visit. You can edit it to switch accounts; only a successful sign-in replaces the saved email. This does not sign you in automatically or save passwords/session tokens in browser storage. Refresh with **Ctrl+Shift+R** to get this update; no server restart is needed. See the [remembered email checks](TEAM_SETUP_GUIDE.md#remembered-staff-email).
...
Restart the server after updating, hard-refresh both pages, and sign in again. See the [teammate guide](TEAM_SETUP_GUIDE.md#preferred-contact-and-project-owner-tags).
